...
//  print "get away! 2";
  $query5 = sprintf("update regist_table set reg_id = '%s', longi = %f, lati = %f where android_id = '%s';", $regid, $longi,$lati,$kekka[2]);
  mysql_query($query5);
  mysql_close($s);
...
if($num ==0 and $kekka == FALSE){
  ...
  $query2 = sprintf("insert into regist_table values(NULL,'%s', '%s', %f, %f);",$regid,$andid, $longi, $lati);
//print $query2;
  mysql_query($query2);
...
$query3 = sprintf("update regist_table set terminal_num = %d, reg_id = '%s', longi = %f, lati = %f where android_id = '%s';",$num,$regid,$longi,$lati,$kekka[2]);
mysql_query($query3);
mysql_close($s);
//print $num;
return($num);
